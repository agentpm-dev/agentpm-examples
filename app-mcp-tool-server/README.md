...
zack-server is a local HTTP MCP server generated from the published `@zack/mcp-tool-server` workflow template.
...
curl -X POST http://127.0.0.1:7331/mcp \
  ...
  -d '{
    ...
    "params": {
      ...
      "arguments": {
        "path": "/absolute/path/to/sample-inputs/hello.md",
        "to_format": "markdown"
      }
    }
  }'
